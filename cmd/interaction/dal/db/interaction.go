...
func DeleteVideoLike(ctx context.Context, videoId, userId int64) error {
	if err := DB.WithContext(ctx).Where("video_id = ? And user_id = ?", videoId, userId).Delete(&model.VideoLike{}).Error; err != nil {
		return err
	}
	return nil
}
...
func DeleteCommentLike(ctx context.Context, commentId, UserId int64) error {
	if err := DB.Where("comment_id = ? And user_id = ?", commentId, UserId).Delete(&model.CommentLike{}).Error; err != nil {
		hlog.Info(err)
		return err
	}
	return nil
...
func GetVideoLikeListByUserId(ctx context.Context, userId, pageNum, pageSize int64) (*[]string, error) {
	list := make([]string, 0)
	if err := DB.WithContext(ctx).Model(&model.VideoLike{}).Where("user_id = ?", userId).Select("video_id").Limit(int(pageSize)).Offset(int(pageNum-1) * int(pageSize)).Scan(list).Error; err != nil {
		return nil, err
	}
	return &list, nil
}
