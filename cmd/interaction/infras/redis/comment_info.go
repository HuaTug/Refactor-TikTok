...
func DeleteCommentAndAllAbout(commentId int64) error {
	...
	commentPipe.Del("l_comment:" + fmt.Sprint(commentId))
	commentPipe.Del("nl_comment:" + fmt.Sprint(commentId))
	for _, item := range *childList {
		commentPipe.Del("l_comment:" + fmt.Sprint(item))
		commentPipe.Del("nl_comment:" + fmt.Sprint(item))
	}
